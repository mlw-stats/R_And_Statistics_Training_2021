...
author: "Marc Henrion"
date: "19, 21, 26, 28 October & 2, 4 November 2021"
output:
  html_document:
    toc: true
    toc_depth: 4
...
ymd_hms("2021-06-04 10:00:00")

ym("2021-06")

ymd("2021-06-04") - ymd("2021-06-02")
```
...
# Sessions 4 & 5 - Basic statistical analysis
...
* One-way ANOVA.
* Kruskal-Wallis test.
* Tow-sample test for proportions.
...
The p-value is `r round(digits=4,t.test(x=vec1,y=vec2)$p.value)` which is less than 0.05. So there is enough evidence, at the 5% significance threshold, to reject the null hypothesis that the 2 flower species have the same sepal width.
...
The p-value is `r round(digits=4,wilcox.test(x=vec1,y=vec2)$p.value)` which, while slightly different than the one from the t-test, us also less than 0.05. So we conclude there is enough evidence to reject the null hypothesis of equal sepal width distributions for the 2 species of flowers.
...
ANOVA is an acronym for **an**alysis **o**f **va**riance. It generalises the t-test to more than 2 groups. It is a parametric test, assuming normality of sample means, and the test statistics uses the F distribution, hence one-way ANOVA is an F-test.
...
$$
Where $\epsilon_i$ are error or residual terms.
